...
<template>
    <div class="min-h-screen flex bg-gray-50 dark:bg-gray-900">
        <!-- Desktop Sidebar -->
        <aside
            class="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-col fixed inset-y-0 left-0 z-30 hidden lg:flex"
        >
            <!-- Logo -->
            <div class="h-16 flex items-center px-6 border-b border-gray-200 dark:border-gray-700">
                <Link href="/dashboard" class="text-xl font-semibold text-gray-900 dark:text-white">
                    Copy Company
                </Link>
            </div>
            ...
            <div class="px-3 py-4 border-t border-gray-200 dark:border-gray-700">
                ...
                >
                    <!-- User Group icon -->
                    <svg v-if="link.icon === 'user-group'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 p-4">
                ...
                <div v-if="brand" class="relative mb-3">
                    <button
                        ...
                        class="w-full flex items-center justify-between px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                    >
                        <span class="truncate">{{ brand.name }}</span>
                        <svg class="w-4 h-4 shrink-0 transition-transform" :class="{ 'rotate-180': showBrandSwitcher }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div
                        ...
                        class="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 max-h-48 overflow-y-auto"
                    >
                        <button
                            v-for="b in brands"
                            :key="b.id"
                        ...
                        >
                            ...
                            <svg v-if="b.id === brand?.id" class="w-4 h-4 text-primary-600 dark:text-white shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                            </svg>
                        </button>
                        <Link
                            href="/brands/create"
                            class="flex items-center gap-2 px-3 py-2 text-sm text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-gray-700"
                        >
                ...
                <div v-if="user" class="flex items-center gap-3 mb-3">
                    <div class="w-8 h-8 bg-primary-100 dark:bg-primary-900 rounded-full flex items-center justify-center shrink-0">
                ...
                <div class="flex items-center gap-2">
                    <button
                        @click="toggleDark"
                        class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                    >
                    ...
                    <button
                        @click="logout"
                        class="flex items-center justify-center p-2 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                        title="Log out"
                    >
        ...
        <!-- Mobile Header -->
        <div class="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 z-20 flex items-center px-4">
            <button
            ...
            <Link href="/dashboard" class="ml-4 text-lg font-semibold text-gray-900 dark:text-white">
                Copy Company
            </Link>
        </div>
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="mobileMenuOpen" class="lg:hidden fixed inset-0 z-40">
                    <!-- Backdrop -->
                    <div class="fixed inset-0 bg-gray-600/75 dark:bg-gray-900/80" @click="closeMobileMenu" />
                    ...
                    >
                        <aside
                            v-if="mobileMenuOpen"
                            class="fixed inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 flex flex-col shadow-xl"
                        >
                            <!-- Logo & Close -->
                            <div class="h-16 flex items-center justify-between px-6 border-b border-gray-200 dark:border-gray-700">
                                <Link href="/dashboard" class="text-xl font-semibold text-gray-900 dark:text-white" @click="closeMobileMenu">
                                    Copy Company
                                </Link>
                                <button
                            ...
                            <div class="border-t border-gray-200 dark:border-gray-700 p-4">
                                ...
                                <div v-if="brand" class="mb-3">
                                    <select
                                        ...
                                        class="w-full px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg focus:ring-2 focus:ring-primary-500"
                                    >
                                        <option v-for="b in brands" :key="b.id" :value="b.id" :selected="b.id === brand?.id">
                                            {{ b.name }}
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    <button
                                        @click="toggleDark"
                                        class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg"
                                    >
                                        <svg v-if="isDark" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                                        </svg>
                                    ...
                                    <button
                                        @click="logout"
                                        class="flex items-center justify-center p-2 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg"
                                        title="Log out"
                                    >
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
        ...
        <div class="flex-1 lg:ml-64">
            <main class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8 pt-24 lg:pt-8">
                <slot />
            </main>
        </div>
    </div>
