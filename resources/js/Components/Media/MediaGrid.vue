...
<template>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <div
            ...
            :key="item.id"
            :class="[
                'relative group bg-white rounded-lg overflow-hidden shadow-sm border-2 transition cursor-pointer',
                selectedIds.includes(item.id) ? 'border-primary-500 ring-2 ring-primary-200' : 'border-transparent hover:border-gray-300'
            ]"
        ...
        >
            ...
            <button
                ...
                class="absolute top-2 right-2 w-6 h-6 rounded-full bg-white/80 text-gray-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition hover:bg-white"
                title="Edit alt text"
            >
