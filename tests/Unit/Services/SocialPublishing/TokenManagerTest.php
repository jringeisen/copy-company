...
class TokenManagerTest extends TestCase
{
    ...
    {
        $credentials = $this->tokenManager->getCredentials($this->brand, 'twitter');
    ...
    public function test_store_credentials_saves_to_brand(): void
    {
        ...
        $this->tokenManager->storeCredentials($this->brand, 'twitter', $credentials);

        $this->brand->refresh();
        $storedCredentials = $this->tokenManager->getCredentials($this->brand, 'twitter');
    ...
    public function test_remove_credentials_removes_platform(): void
    {
        ...
        $this->assertTrue($this->tokenManager->isConnected($this->brand, 'twitter'));

        $this->tokenManager->removeCredentials($this->brand, 'twitter');

        $this->assertFalse($this->tokenManager->isConnected($this->brand, 'twitter'));
    }
    ...
    {
        $this->tokenManager->storeCredentials($this->brand, 'twitter', ['access_token' => 'token1']);
    ...
    public function test_get_connection_info_returns_safe_data(): void
    {
        ...
        $info = $this->tokenManager->getConnectionInfo($this->brand, 'twitter');
    ...
    public function test_refresh_if_needed_calls_refresh_when_token_needs_refresh(): void
    {
        ...
        $result = $this->tokenManager->refreshIfNeeded($this->brand, 'twitter', $publisher);
    ...
    public function test_multiple_platforms_stored_independently(): void
    {
        ...
        $twitterCreds = $this->tokenManager->getCredentials($this->brand, 'twitter');
