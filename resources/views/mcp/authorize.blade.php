        ...
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
            ...
            <div class="p-6 pt-0 space-y-4">
                ...
                @if(isset($brands) && $brands->count() > 0)
                    <div class="space-y-2">
                        <label for="brand_id" class="text-sm font-medium">Select Brand:</label>
                        <p class="text-xs text-muted-foreground">Choose which brand the AI agent will have access to.</p>
                        <select name="brand_id" id="brand_id" form="authorizeForm" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" required>
                            @foreach($brands as $brand)
                                <option value="{{ $brand->id }}" {{ $user->currentBrand()?->id === $brand->id ? 'selected' : '' }}>
                                    {{ $brand->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                @elseif(isset($brands) && $brands->count() === 0)
                    <div class="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                        <p class="text-sm text-destructive">No brands available. Please create a brand first.</p>
                    </div>
                @endif
