...
const statusBadgeClass = computed(() => {
    ...
    if (isPending.value) return 'bg-[#a1854f]/20 text-[#a1854f]';
    if (isFailed.value) return 'bg-red-100 text-red-800';
    return 'bg-[#0b1215]/10 text-[#0b1215]/70';
});
        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-2xl border border-[#0b1215]/10 p-6 mb-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-medium text-[#0b1215]/50">Current Sending Address</h3>
                        <p class="text-lg font-semibold text-[#0b1215] mt-1">{{ currentFromAddress }}</p>
                    </div>
                    <span
            ...
            <div v-if="!hasCustomDomain" class="bg-white rounded-2xl border border-[#0b1215]/10 p-8">
                <div class="text-center mb-8">
                    <div class="mx-auto w-16 h-16 bg-[#a1854f]/10 rounded-full flex items-center justify-center mb-4">
                        <svg class="w-8 h-8 text-[#a1854f]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                    </div>
                    <h2 class="text-xl font-semibold text-[#0b1215]">Add Your Custom Email Domain</h2>
                    <p class="text-[#0b1215]/60 mt-2 max-w-md mx-auto">
                ...
                <form @submit.prevent="submitDomain" class="space-y-6 max-w-md mx-auto">
                    <!-- Domain Input -->
                    <div>
                        <label for="domain" class="block text-sm font-medium text-[#0b1215]">Domain Name</label>
                        <input
                            id="domain"
                    ...
                    <!-- From Address Input -->
                    <div>
                        <label for="from_address" class="block text-sm font-medium text-[#0b1215]">From Address</label>
                        ...
                        <p class="mt-1 text-sm text-[#0b1215]/50">This will be the "from" part of your email address</p>
            ...
            <div v-else class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-[#0b1215]/10 p-6">
                    <div class="flex items-start justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-[#0b1215]">{{ brand.custom_email_domain }}</h3>
                            <p class="text-sm text-[#0b1215]/50 mt-1">
                ...
                <div v-if="(isPending || isFailed) && brand.email_domain_dns_records" class="bg-white rounded-2xl border border-[#0b1215]/10 p-6">
                    <h3 class="text-lg font-semibold text-[#0b1215] mb-4">DNS Records Required</h3>
                    <p class="text-sm text-[#0b1215]/60 mb-6">Add these DNS records to your domain's DNS settings. Verification usually takes 15-60 minutes after records are added.</p>

                    <!-- Verification TXT Record -->
                    <div v-if="brand.email_domain_dns_records.verification" class="mb-6">
                        <h4 class="text-sm font-medium text-[#0b1215] mb-2">Domain Verification (TXT Record)</h4>
                        <div class="bg-[#f7f7f7] rounded-xl p-4 space-y-2">
                            <div class="flex items-center justify-between">
                                <div>
                                    <span class="text-xs text-[#0b1215]/50">Type:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215]">{{ brand.email_domain_dns_records.verification.type }}</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <span class="text-xs text-[#0b1215]/50">Name:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ brand.email_domain_dns_records.verification.name }}</span>
                                </div>
                                <button
                                    @click="copyToClipboard(brand.email_domain_dns_records.verification.name)"
                                    class="ml-2 p-1 text-[#0b1215]/40 hover:text-[#0b1215]/70"
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <span class="text-xs text-[#0b1215]/50">Value:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ brand.email_domain_dns_records.verification.value }}</span>
                                </div>
                                <button
                    ...
                    <div v-if="brand.email_domain_dns_records.dkim && brand.email_domain_dns_records.dkim.length > 0" class="mb-6">
                        <h4 class="text-sm font-medium text-[#0b1215] mb-2">DKIM Records (3 CNAME Records)</h4>
                        <div class="space-y-3">
                            <div
                                v-for="(record, index) in brand.email_domain_dns_records.dkim"
                                :key="index"
                                class="bg-[#f7f7f7] rounded-xl p-4 space-y-2"
                            >
                                <div class="flex items-center justify-between">
                                    <div>
                                        <span class="text-xs text-[#0b1215]/50">Type:</span>
                                        <span class="ml-2 text-sm font-mono text-[#0b1215]">{{ record.type }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center justify-between">
                                    <div class="flex-1 min-w-0">
                                        <span class="text-xs text-[#0b1215]/50">Name:</span>
                                        <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ record.name }}</span>
                                    </div>
                                    <button
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-1 min-w-0">
                                        <span class="text-xs text-[#0b1215]/50">Value:</span>
                                        <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ record.value }}</span>
                                    </div>
                                    <button
                    ...
                    <div v-if="brand.email_domain_dns_records.spf">
                        <h4 class="text-sm font-medium text-[#0b1215] mb-2">SPF Record (TXT Record)</h4>
                        <div class="bg-[#f7f7f7] rounded-xl p-4 space-y-2">
                            <div class="flex items-center justify-between">
                                <div>
                                    <span class="text-xs text-[#0b1215]/50">Type:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215]">{{ brand.email_domain_dns_records.spf.type }}</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <span class="text-xs text-[#0b1215]/50">Name:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ brand.email_domain_dns_records.spf.name }}</span>
                                </div>
                                <button
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <span class="text-xs text-[#0b1215]/50">Value:</span>
                                    <span class="ml-2 text-sm font-mono text-[#0b1215] break-all">{{ brand.email_domain_dns_records.spf.value }}</span>
                                </div>
                                <button
                        ...
                        <p class="mt-2 text-xs text-[#0b1215]/50">
                            Note: If you already have an SPF record, add <code class="bg-[#0b1215]/5 px-1 rounded">include:amazonses.com</code> to your existing record.
                        </p>
                    </div>
                </div>

                <!-- From Address Settings (if verified) -->
                <div v-if="isVerified" class="bg-white rounded-2xl border border-[#0b1215]/10 p-6">
                    <h3 class="text-lg font-semibold text-[#0b1215] mb-4">From Address</h3>
                    <form @submit.prevent="updateFrom" class="space-y-4">
                        <div>
                            <label for="edit_from_address" class="block text-sm font-medium text-[#0b1215]">Email Address</label>
                            <div class="mt-1 flex rounded-xl overflow-hidden border border-[#0b1215]/20">
                                <input
                ...
                <div v-if="isVerified" class="bg-green-50 border border-green-200 rounded-2xl p-6">
